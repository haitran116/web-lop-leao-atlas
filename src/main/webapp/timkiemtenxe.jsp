...
<body>
	<%
		...
		Hangxe hangxe = (Hangxe) request.getAttribute("hangxe");
		ArrayList<Lopxe> ket_qua_tim_kiem = (ArrayList<Lopxe>) request.getAttribute("ket_qua_tim_kiem");	
...
<div class="container">
		<div class="col-12 d-flex justify-content-center">
			<p style="border:1px solid black; padding: 10px; font-size: 20px; margin-top: 40px; white-space: pre-line;"><%=hangxe.getCuthe() %></p>
		</div>
	</div>
		...
		<div class="container">			
			<div class="select-sapxep">	
				<div style="margin-right: auto;" class="nhan">
					<p><%=(ket_qua_tim_kiem.size() > 0)?"Tìm thấy "+ket_qua_tim_kiem.size()+" sản phẩm.":"Không tìm thấy sản phẩm nào cho xe "+hangxe.getTenxe()+"" %></p>
				</div>				
			...
			<div class="row">				
				<%								
					// đếm sô số lốp
					int countLop = gdt.soluong_lop(ket_qua_tim_kiem);
				...
				<div class="col-lg-3 col-md-3 text-center">
					<div class="single-product-item">
						<div class="product-image">
							<a href="san-pham/<%=cn.url_san_pham(ket_qua_tim_kiem.get(i).getThuonghieu(), ket_qua_tim_kiem.get(i).getSize(), ket_qua_tim_kiem.get(i).getTensanpham()) %>"><img
								src="assets/img/products/anhlop/<%=ket_qua_tim_kiem.get(i).getNamefolder() %>/main.jpg" alt=""></a>
						</div>
						<% String ten = ket_qua_tim_kiem.get(i).getSize() + " " + ket_qua_tim_kiem.get(i).getTensanpham(); %>
						
						<h6><%=cn.Rut_Gon_Ten_Lop(ten) %> </h6>																		
						
						<p class="product-price">
							<span> <%=ket_qua_tim_kiem.get(i).getThuonghieu() %>
							</span>
							<%=cn.chuan_hoa_gia(ket_qua_tim_kiem.get(i).getGiaban())+" VNĐ" %>
						</p>
						
						<div>
							<a href="CookieCart?masp=<%=ket_qua_tim_kiem.get(i).getMasp() %>&num=1&page=<%=index_page %>&jsp=timkiemtenxe&tenxe=<%=hangxe.getTenxe() %>" onclick="themvaogio()"
							class="cart-btn"><i class="fas fa-shopping-cart"></i> Thêm vào giỏ</a> 	
						</div>					
					</div>
				</div>
				<% }} %>
			...
			<div class="row">
				<div class="col-lg-12 text-center">
					<div class="pagination-wrap">
						<ul>
							<li><a href="loc_theo_hang_xe?tenxe=<%=hangxe.getTenxe() %>&page=<%=(index_page == 1)?index_page:index_page-1 %>"><<</a></li>	
					
							<%													
								for(int i = 1; i <= pagenumber; i++){
							%>							
							<li><a class="<%=(i == index_page)?"active":"" %>" href="loc_theo_hang_xe?tenxe=<%=hangxe.getTenxe() %>&page=<%=i %>"><%=i %></a></li>						
							<% } %>
							<li><a href="loc_theo_hang_xe?tenxe=<%=hangxe.getTenxe() %>&page=<%=(index_page == pagenumber)?index_page:index_page+1 %>">>></a></li>
						</ul>
					</div>
				</div>
			</div>
	...
	<script type="text/javascript">
		function click_sapxep(val) {
			var hangxe = '<%=hangxe.getTenxe() %>';
			var url = "loc_theo_hang_xe?tenxe="+hangxe+"&sapxep="+val;
		    window.location.href = url;
		}
	</script>
