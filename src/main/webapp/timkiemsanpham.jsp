...
<body>
	<%
		...
		ArrayList<Lopxe> lopxe = (ArrayList<Lopxe>) request.getAttribute("lopxe");
		...
		<div class="container">
			<div class="select-sapxep">	
				<div style="margin-right: auto;" class="nhan">	
					<p>Tìm thấy <%=lopxe.size() %> sản phẩm.</p>	
				</div>
				...
				<div class="col-lg-3 col-md-3 text-center">
					<div class="single-product-item">
						<div class="product-image">
							<a href="san-pham/<%=cn.url_san_pham(lop.getThuonghieu(), lop.getSize(), lop.getTensanpham()) %>"><img
								src="assets/img/products/anhlop/<%=lop.getNamefolder() %>/main.jpg" alt=""></a>
						</div>
						<% String ten = lop.getSize() + " " + lop.getTensanpham(); %>
						<h6><%=cn.Rut_Gon_Ten_Lop(ten) %> </h6>
						<p class="product-price">
							<span> <%=lop.getSize() %>
							</span>
							<%=cn.chuan_hoa_gia(lop.getGiaban())+" VNĐ" %>
						</p>
						<div>
							<a href="CookieCart?masp=<%=lop.getMasp() %>&num=1&page=1&jsp=timkiemsanpham" onclick="themvaogio()"
	...
	<script type="text/javascript">
		function click_sapxep(val) {
			var size = '<%=lopxe.get(0).getSize() %>';
			var url = "leao-atlas-michelin-loc-theo-size?sizelop="+size+"&sapxep="+val;
		    window.location.href = url;
		}
	</script>	
