...
<body>
	<%
		...
		String thuonghieu = (String) request.getAttribute("thuonghieu");
		if(thuonghieu == null){
			thuonghieu = "all";
		}
	...
	<div class="product-filters">
		<ul>
			<li class="<%=(thuonghieu.equals("la")?"active":"") %>" onclick="click_thuonghieu(this)">Lốp Leao</li>
			<li class="<%=(thuonghieu.equals("al")?"active":"") %>" onclick="click_thuonghieu(this)">Lốp Atlas</li>
			<li class="<%=(thuonghieu.equals("mc")?"active":"") %>" onclick="click_thuonghieu(this)">Lốp Michelin</li>
			<li class="<%=(thuonghieu.equals("all")?"active":"") %>" onclick="click_thuonghieu(this)">Tất cả</li>
		</ul>
	</div>	
			...
			<div class="row">
				...
				<div class="col-lg-3 col-md-6 text-center">
					<div class="single-product-item">
						...
						<div>
							<a
								href="CookieCart?masp=<%=lop.get(i).getMasp()%>&th=<%=thuonghieu %>&num=1&page=<%=index_page%>&jsp=sanpham-hang&sapxep=<%=sapxep %>"
								onclick="themvaogio()" class="cart-btn"><i
			...
			<div class="row">
				<div class="col-lg-12 text-center">
					<div class="pagination-wrap">
						<ul>
							<li><a href="leao-atlas-michelin?page=<%=(index_page == 1)?index_page:index_page-1 %>&th=<%=lop.get(0).getMasp() %>&sapxep=<%=sapxep %>"><<</a></li>	
							<li><a <%=(index_page == 1)?"hidden=''":"" %> href="leao-atlas-michelin?page=1&th=<%=lop.get(0).getMasp() %>&sapxep=<%=sapxep %>">1</a></li>							
							<%
								String show = "active";
							...
							%>							
							<li><a class="<%=show %>" href="leao-atlas-michelin?page=<%=i %>&th=<%=lop.get(0).getMasp() %>&sapxep=<%=sapxep %>"><%=i %></a></li>						
							<% } %>
							<li><a >...</a></li>
							<li><a class="<%=(tmp_index_page == pagenumber)?"active":"" %>" href="leao-atlas-michelin?page=<%=pagenumber %>&th=<%=lop.get(0).getMasp() %>&sapxep=<%=sapxep %>"><%=pagenumber %></a></li>
							<li><a href="leao-atlas-michelin?page=<%=(tmp_index_page == pagenumber)?tmp_index_page:tmp_index_page+1 %>&th=<%=lop.get(0).getMasp() %>&sapxep=<%=sapxep %>">>></a></li>	
						</ul>
					</div>
				</div>
			</div>
	...
	<script>
		...
		function click_sapxep(val) {
			var thuonghieu_ = '<%= thuonghieu %>';
			var url = "leao-atlas-michelin?page=1&th="+thuonghieu_+"&sapxep="+val;
		    window.location.href = url;
		}
	</script>	
